...
let lucyVoiceConfig: VoiceConfig;
try {
  ...
  lucyVoiceConfig = config.voices.lucy;
} catch (e) {
  // Fallback to hardcoded Lucy voice config
  lucyVoiceConfig = {
...
async function main() {
  ...
  // Send notification with voice (using Lucy's voice from config)
  await sendNotification({
    title: 'Lucy Context',
    message: message,
    voice_enabled: true,
    voice_name: lucyVoiceConfig.voice_name,
    rate: lucyVoiceConfig.rate_wpm,
    priority: 'low',
  });
